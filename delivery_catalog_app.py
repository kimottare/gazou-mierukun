...
st.markdown("""
    ...
    /* ==========================================
       📱 iPhone Safari/Edge用：2列強制・絶対命令
       ========================================== */
    @media screen and (max-width: 800px) {
        /* Streamlitの親コンテナのFlex-Wrapを強制 */
...
def guess_column_index(columns, keywords, default_idx=0, exclude_keywords=[]):
    for keyword in keywords:
        for idx, col in enumerate(columns):
            c_low = str(col).lower()
            if keyword in c_low:
                if not any(ex in c_low for ex in exclude_keywords):
                    return idx
    return default_idx
...
with st.sidebar:
    ...
    if st.session_state.generated:
        ...
        # 🌟 修正：BS（カテゴリー）にサイズが混ざらないよう、数字を含むものを除外する
        unique_bs = sorted(list(set([
            str(i["bs"]).strip() for i in items 
            if i.get("bs") and not any(char.isdigit() for char in str(i["bs"])) and len(str(i["bs"])) > 2
        ])))
    ...
    if uploaded_file:
        try:
            ...
            with st.expander("📋 列割り当て確認", expanded=True):
                ...
                with c3:
                    # 🌟 修正：BSの自動検知時に「size」を含む列を避ける
                    bs_col = st.selectbox("BS (カテゴリー)", columns if 'bs' in [str(c).lower() for c in columns] else ["(なし)"]+columns, index=guess_column_index(columns, ['bs', 'category', '部門'], exclude_keywords=['size', 'サイズ'])+1)
                    # 🌟 修正：Statusの初期値を「列12」付近にする（もし存在すれば）
                    status_idx = 11 if len(columns) >= 12 else len(columns)-1
                    status_col = st.selectbox("Status", ["(なし)"] + columns, index=status_idx + 1)
...
if st.session_state.generated:
    ...
    num_cols = 5 if is_print_mode else 2 # 🌟 デフォルトをスマホ意識して2列に
